...
const messagetable=require('../models/msgtable')
...
exports.AddGroupchatMessage=(async (req,res)=>{
 ...
 const response=await messagetable.create({
    ...
    res.send({success:true,msg:"successfully inserted", response: response})
  } catch (error) {
    res.send({success:false,msg:error})
  }
...
exports.AddNewUserToGroup=(async (req,res)=>{
  ...
  for (let index = 0; index < req.body.groupUseridArray.length; index++) {
    usergroup.create({
        grouptableId:req.body.groupid,
        userdetailId:req.body.groupUseridArray[index]
    })
}
...
exports.RemoveUserFromGroup=(async (req,res)=>{
  console.log()
    await usergroup.destroy({where:{
      grouptableId:req.body.groupid,
      userdetailId:req.body.groupUseridArray
    }
  ...
  try {
    ...
    for (let index = 0; index < req.body.groupUseridArray.length; index++) {
      await usergroup.update(
        { isAdmin: true },
        { where: { userdetailId:req.body.groupUseridArray[index],
                   grouptableId:req.body.groupid} 
                });
